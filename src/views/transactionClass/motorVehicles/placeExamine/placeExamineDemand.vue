...
<script>
  ...
  export default {
    ...
    methods: {
      cancelReverse () {
        let requestData = {
          proposerIdentityCard: this.$store.state.user.identityCard,
          sourceOfCertification: this.$store.state.core.source
        }
        axios.post(inspectionDeclarationQuery, requestData).then(json => {
          if (json.code === '0000') {
